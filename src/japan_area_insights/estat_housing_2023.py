...
def _seed(conn) -> None:
    datasets = [
        ...
        ("estat_housing_2023_structure_age", "政府統計の総合窓口 e-Stat", TABLE_STRUCTURE_AGE, "housing", "2023住宅・土地統計 構造×建築時期", "2023-10", "municipality", "extended", 1, "表6-3-2。全国・都道府県・市区町村。木造と非木造が全住宅を分割し、RC/SRC等と鉄骨造は非木造の内訳。"),
    ]
    conn.executemany(
        """
    ...
    definitions = [
        ...
        ("housing2023.wooden_share", "housing", "木造住宅比率", "%", "neutral", "ward", "estat_housing_2023_structure_age", 1, "木造住宅÷（木造＋非木造）"),
        ("housing2023.nonwooden_share", "housing", "非木造住宅比率", "%", "neutral", "ward", "estat_housing_2023_structure_age", 1, "非木造住宅÷（木造＋非木造）"),
        ("housing2023.rc_share", "housing", "RC・SRC等住宅比率", "%", "neutral", "ward", "estat_housing_2023_structure_age", 1, "鉄筋・鉄骨コンクリート造住宅÷（木造＋非木造）"),
        ("housing2023.steel_share", "housing", "鉄骨造住宅比率", "%", "neutral", "ward", "estat_housing_2023_structure_age", 1, "鉄骨造住宅÷（木造＋非木造）"),
    ]
    conn.executemany(
        """
...
def _fetch_tenure_age(client: EStatClient, conn, area_ids: list[str]) -> int:
    ...
    total_age = _total(classes[age_dim])
    total_tenure = _total(classes[tenure_dim])
    if not item or not total_age or not total_tenure:
        return 0
    tenure_codes = {
    ...
    age_codes = {
        label: _code(classes[age_dim], lambda value, wanted=_norm(label): value == wanted)
        for label in (
            "1970年以前",
            "1971～1980年",
            "1981～1990年",
    ...
    for row in _values(payload):
        ...
        age = str(row.get(f"@{age_dim}") or "")
        tenure = str(row.get(f"@{tenure_dim}") or "")
        number = _number(row.get("$"))
        if area_id in area_ids and number is not None:
            values[(area_id, age, tenure)] = number

    written = 0
    for area_id in area_ids:
        total = values.get((area_id, total_age, total_tenure))

        def tenure_value(name: str) -> float | None:
            code = tenure_codes.get(name)
            return values.get((area_id, total_age, code)) if code else None

        def age_value(label: str) -> float | None:
            code = age_codes.get(label)
            return values.get((area_id, code, total_tenure)) if code else None

        pre1980_parts = (age_value("1970年以前"), age_value("1971～1980年"))
        pre2000_parts = (*pre1980_parts, age_value("1981～1990年"), age_value("1991～2000年"))
        post2011_parts = (age_value("2011～2020年"), age_value("2021～2023年9月"))
        pre1980 = sum(v for v in pre1980_parts if v is not None) if any(v is not None for v in pre1980_parts) else None
        pre2000 = sum(v for v in pre2000_parts if v is not None) if any(v is not None for v in pre2000_parts) else None
        post2011 = sum(v for v in post2011_parts if v is not None) if any(v is not None for v in post2011_parts) else None
        latest = age_value("2021～2023年9月")
        metrics = [("housing2023.total_housing", total)]
        for key, name in (
            ("housing2023.owner_occupied_share", "owner"),
            ("housing2023.rental_share", "rental"),
            ("housing2023.private_rental_share", "private"),
            ("housing2023.public_rental_share", "public"),
            ("housing2023.ur_public_corp_rental_share", "ur"),
        ):
            number = tenure_value(name)
            metrics.append((key, number / total * 100 if number is not None and total else None))
        metrics.extend(
            [
                ("housing2023.pre1980_share", pre1980 / total * 100 if pre1980 is not None and total else None),
                ("housing2023.pre2000_share", pre2000 / total * 100 if pre2000 is not None and total else None),
                ("housing2023.post2011_share", post2011 / total * 100 if post2011 is not None and total else None),
        ...
        for key, value in metrics:
            written += _write(
                ...
                source_id,
                "2023住宅・土地統計調査 表5-3。市区町村粒度。",
            )
    return written


def _fetch_structure(client: EStatClient, conn, area_ids: list[str]) -> int:
    meta = client.get_meta_info(TABLE_STRUCTURE_AGE)
    classes = _class_map(_meta_class_objects(meta))
    area_dim = _dimension_containing_codes(classes, area_ids)
    item_dim = _dimension(classes, ("住宅数",), {area_dim})
    age_dim = _dimension(classes, ("1970年以前", "2021～2023年9月"), {area_dim, item_dim})
    ...
    if not item or not total_age or not structure_codes["wood"] or not structure_codes["nonwood"]:
        return 0
    params = {
        _filter_name(area_dim): ",".join(area_ids),
        _filter_name(item_dim): item,
        _filter_name(age_dim): total_age,
        _filter_name(structure_dim): ",".join(code for code in structure_codes.values() if code),
        "metaGetFlg": "Y",
        "cntGetFlg": "N",
    }
    ...
    for row in _values(payload):
        area_id = str(row.get(f"@{area_dim}") or "")
        structure = str(row.get(f"@{structure_dim}") or "")
        number = _number(row.get("$"))
        if area_id in area_ids and number is not None:
            values[(area_id, structure)] = number

    written = 0
    for area_id in area_ids:
        wood = values.get((area_id, structure_codes["wood"]))
        nonwood = values.get((area_id, structure_codes["nonwood"]))
        total = wood + nonwood if wood is not None and nonwood is not None else None
        for key, name in (
            ("housing2023.wooden_share", "wood"),
            ("housing2023.nonwooden_share", "nonwood"),
            ("housing2023.rc_share", "rc"),
            ("housing2023.steel_share", "steel"),
        ):
            code = structure_codes.get(name)
            number = values.get((area_id, code)) if code else None
            value = number / total * 100 if number is not None and total else None
            written += _write(
                conn,
                area_id,
                key,
                round(value, 3) if value is not None else None,
                source_id,
                "2023住宅・土地統計調査 表6-3-2。市区町村粒度。木造＋非木造を全住宅として構成比を計算。",
            )
    return written
