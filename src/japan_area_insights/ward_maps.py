...
def build_ward_mesh_payload(conn, area_id: str) -> dict[str, Any]:
    ...
    for mesh_id, values in by_mesh.items():
        ...
        retention = (
            round(pop_2045 / pop_2025 * 100.0, 2)
            if pop_2025 not in (None, 0) and pop_2045 is not None
            else None
        )
        meshes.append(
            {
    ...
    return {
        ...
        "summary": {
            ...
            "retention_2045_area": (
                round(sum(values_2045) / sum(values_2025) * 100.0, 2)
                if values_2025 and values_2045 and sum(values_2025) != 0
                else None
            ),
            "retention_2045_mesh_median": (
                round(sorted(retention_values)[len(retention_values) // 2], 2)
                if retention_values
                else None
            ),
        },
...
def export_ward_mesh_maps(db_path: str | Path, output_dir: str | Path) -> None:
    ...
    with connect(db_path) as conn:
        ...
        for area_id in area_ids:
            ...
            (area_dir / "mesh250.json").write_text(
