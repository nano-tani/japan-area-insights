...
def _dimension(classes: Mapping[str, Mapping[str, str]], labels: tuple[str, ...], excluded: set[str] | None = None) -> str:
...
def _seed(conn) -> None:
    conn.executemany(
        ...
        [
            (
                "estat_census_2020_nationality", "政府統計の総合窓口 e-Stat", FOREIGN_NATIONALITY_ID,
                "demographics", "2020年国勢調査 国籍別人口", "2020-10", "municipality", "extended", 1,
                "人口等基本集計 表44-1。男女計・市区町村粒度。",
            ),
            (
                "estat_census_2020_household_size", "政府統計の総合窓口 e-Stat", HOUSEHOLD_SIZE_ID,
                "demographics", "2020年国勢調査 一般世帯の世帯人員", "2020-10", "municipality", "extended", 1,
...
def fetch_nationality(client: EStatClient, conn, area_ids: list[str]) -> int:
    ...
    nationality_dim = _dimension(classes, ("外国人", "中国", "フィリピン", "ベトナム", "ネパール"), {area_dim})
    ...
    source_id = _source_id(conn, title="2020年国勢調査 国籍別人口", dataset_id=FOREIGN_NATIONALITY_ID, payload=payload)
    ...
    for area_id in area_ids:
        ...
        for key, value in metrics:
            written += _write(conn, area_id, key, value, source_id, "2020年国勢調査 表44-1。男女計・市区町村粒度。")
    return written
...
def fetch_household_size(client: EStatClient, conn, area_ids: list[str]) -> int:
    ...
    classes = _class_map(_meta_class_objects(meta))
    area_dim = _dimension_containing_codes(classes, area_ids)
    size_dim = _dimension(classes, ("世帯人員が1人", "世帯人員が2人", "世帯人員が4人"), {area_dim})
    ...
    four_plus_codes = [
        ...
        if _norm(label) in {
            "世帯人員が4人", "世帯人員が5人", "世帯人員が6人", "世帯人員が7人",
            "世帯人員が8人", "世帯人員が9人", "世帯人員が10人以上",
        }
    ]
    ...
    source_id = _source_id(conn, title="2020年国勢調査 一般世帯の世帯人員", dataset_id=HOUSEHOLD_SIZE_ID, payload=payload)
    values: dict[tuple[str, str], float] = {}
    for row in _values(payload):
        area_id = str(row.get(f"@{area_dim}") or "")
    ...
    for area_id in area_ids:
        ...
        four_plus = sum(v for v in four_plus_values if v is not None) if any(v is not None for v in four_plus_values) else None
        for key, value in (
            ...
            written += _write(conn, area_id, key, value, source_id, "2020年国勢調査 表6-3-1。一般世帯・市区町村粒度。")
    return written
